=begin
Transpose 3x3
A 3 x 3 matrix can be represented by an Array of Arrays in which the main
Array and all of the sub-Arrays has 3 elements. For example:

1  5  8
4  7  2
3  9  6

can be described by the Array of Arrays:

matrix = [
  [1, 5, 8],
  [4, 7, 2],
  [3, 9, 6]
]

An Array of Arrays is sometimes called nested arrays because each of the
inner Arrays is nested inside an outer Array.

To access an element in matrix, you use Array#[] with both the row index
and column index. So, in this case, matrix[0][2] is 8, and matrix[2][1]
is 9. An entire row in the matrix can be referenced by just using one
index: matrix[1] is the row (an Array) [4, 7, 2]. Unfortunately, there's
no convenient notation for accessing an entire column.

The transpose of a 3 x 3 matrix is the matrix that results from
exchanging the columns and rows of the original matrix. For example,
the transposition of the array shown above is:

1  4  3
5  7  9
8  2  6

Write a method that takes a 3 x 3 matrix in Array of Arrays format and
returns the transpose of the original matrix. Note that there is a
Array#transpose method that does this -- you may not use it for this
exercise. You also are not allowed to use the Matrix class from the
standard library. Your task is to do this yourself.

Take care not to modify the original matrix: you must produce a new
matrix and leave the original matrix unchanged.

Examples

matrix = [
  [1, 5, 8],
  [4, 7, 2],
  [3, 9, 6]
]

new_matrix = transpose(matrix)

p new_matrix == [[1, 4, 3], [5, 7, 9], [8, 2, 6]]
p matrix == [[1, 5, 8], [4, 7, 2], [3, 9, 6]]

This program should print "true" twice.

P
input: array(of arrays - 3 x 3 matrix)
output: return an array (transposed)
explicit rules:
write a method
do not modify the original array
transpose the array (switch row and column values)

E
matrix = [
  [1, 5, 8],
  [4, 7, 2],
  [3, 9, 6]
]
new_matrix = transpose(matrix)
p new_matrix == [[1, 4, 3], [5, 7, 9], [8, 2, 6]]
p matrix == [[1, 5, 8], [4, 7, 2], [3, 9, 6]]

D
arrays

A
- create a new empty array of arrays (3 x 3 matrix) transposed
- iterate through the original array
  - iterate through the sub array
    - append the first element to the first subarray in transposed
    - append the second element to the second subarray in transposed
    - append the third element to the third subarray in transposed
=end

def transpose(matrix)
  transposed = Array.new(3) {[]}
  matrix.each do |row|
    row.each_with_index do |element, index|
      transposed[index] << element
    end
  end
  transposed
end

matrix = [
  [1, 5, 8],
  [4, 7, 2],
  [3, 9, 6]
]

new_matrix = transpose(matrix)

p new_matrix == [[1, 4, 3], [5, 7, 9], [8, 2, 6]]
p matrix == [[1, 5, 8], [4, 7, 2], [3, 9, 6]]

=begin
A
- initialize an empty array `transposed`
- iterate through every first element and save to a new array in transposed
- iterate through every second element, save to an array in transposed
- iterate through every 3rd element...
=end
def transpose(matrix)
  transposed = []
  (matrix.size).times do |index|
    column = matrix.map { |row| row[index] }
    transposed << column
  end
  transposed
end

matrix = [
  [1, 5, 8],
  [4, 7, 2],
  [3, 9, 6]
]

new_matrix = transpose(matrix)

p new_matrix == [[1, 4, 3], [5, 7, 9], [8, 2, 6]]
p matrix == [[1, 5, 8], [4, 7, 2], [3, 9, 6]]
matrix = [
  [1, 3, 8],
  [4, 5, 2],
  [3, 2, 6],
  [0, 4, 7]
]

new_matrix = transpose(matrix)

p new_matrix #== [[1, 4, 3], [5, 7, 9], [8, 2, 6]]
p matrix #== [[1, 5, 8], [4, 7, 2], [3, 9, 6]]
# Provided Solution
def transpose(matrix)
  result = []
  (0..2).each do |column_index|
    new_row = (0..2).map { |row_index| matrix[row_index][column_index] }
    result << new_row
  end
  result
end

=begin
Further Exploration - transpose a matrix in place. Write a transpose! method
A
- swap the 3 values using index reference *multiple assignment syntax
=end
def transpose!(matrix)
  matrix[0][1], matrix[1][0] = matrix[1][0], matrix[0][1]
  matrix[0][2], matrix[2][0] = matrix[2][0], matrix[0][2]
  matrix[1][2], matrix[2][1] = matrix[2][1], matrix[1][2]
  matrix
end

matrix = [
  [1, 5, 8],
  [4, 7, 2],
  [3, 9, 6]
]

new_matrix = transpose!(matrix)

p new_matrix == [[1, 4, 3], [5, 7, 9], [8, 2, 6]]
p matrix == [[1, 4, 3], [5, 7, 9], [8, 2, 6]]
