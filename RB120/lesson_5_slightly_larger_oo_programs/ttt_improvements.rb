class Board
  ...
  def winning_marker
    WINNING_LINES.each do |line|
      squares_values = squares.values_at(*line)
      if three_identical_markers?(squares_values)
        return squares_values.first.marker
      end
    end
  ...
  def three_identical_markers?(squares_values)
    markers = squares_values.map(&:marker)
    markers.uniq.size == 1 && squares_values.all?(&:marked?)
  end
end
